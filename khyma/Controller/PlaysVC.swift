...
class PlaysVC: UIViewController {
    ...
    private var bannerAd1: GADBannerView = {
      ...
      banner.adUnitID = AdUnitKeys.PlaysVCbannerUP
    ...
    private var bannerAd2: GADBannerView = {
      let banner = GADBannerView()
      banner.adUnitID = AdUnitKeys.PlaysVCbannerDown
      banner.load(GADRequest())
      return banner
    }()
    ...
    fileprivate func getVideos(page: Int, genreID: String) {
        ...
        Video.endpoint = "\(Defaults.BASE_URL)\(CategoryName.plays)/genre/\(genreID)/\(page)"
    ...
    fileprivate func getVideos(page: Int) {
        ...
        Play.endpoint = "\(Defaults.BASE_URL)\(CategoryName.plays)?page=\(POPULAR_CURRENT_PAGE)&nameEn=&nameAr="
        API<Play>.list { [weak self] result in
            switch result {
            case .success(let data):
                ...
                if page == 1 {
                    ...
                    self?.plays.append(contentsOf: data)
                }
    ...
    fileprivate func setupViews() {
        ...
        playsSliderCollectionView.layout(XW: .leadingAndCenter(nil, 0), YH: .TopAndBottomAndHeight(nil, 60, playsTableView, 0, .fixed(UIDevice.current.userInterfaceIdiom != .pad ? 600 : 1000)))
        playsSliderCollectionView.reloadData()
...
extension PlaysVC: HorizontalPaginationManagerDelegate {
    ...
    func loadMore(completion: @escaping (Bool) -> Void) {
        refreshDelay(2.0) { [weak self] in
            ...
            if self?.CURRENT_GENRE_ID == "" {
                ...
                print("load more for popular plays from page \(currentPage) ...")
...
extension PlaysVC: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.section == 0 {
            return plays.count == 0 ? CGFloat.leastNonzeroMagnitude : (UIDevice.current.userInterfaceIdiom != .pad ? 200 : 300)
        } else {
